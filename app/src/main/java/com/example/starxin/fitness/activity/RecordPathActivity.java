...
import com.example.starxin.fitness.adapter.DbAdapter;
import com.example.starxin.fitness.utils.Util;
import com.example.starxin.fitness.view.PathRecord;
...
public class RecordPathActivity extends Activity implements LocationSource,
		...
		btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (btn.isChecked()) {
					...
					mTraceClient.queryProcessedTrace(2, Util.parseTraceLocationList(record.getPathline()) , LBSTraceClient.TYPE_AMAP, RecordPathActivity.this);
	...
	protected void saveRecord(List<AMapLocation> list, String time) {
		if (list != null && list.size() > 0) {
			...
			Toast.makeText(RecordPathActivity.this, "没有记录到路径", Toast.LENGTH_SHORT)
					.show();
		}
	}
...
//		mAMap.moveCamera(CameraUpdateFactory.zoomTo(18));
		...
		MyLocationStyle my=new MyLocationStyle();
		my.strokeColor(Color.argb(0,0,0,0));
		my.radiusFillColor(Color.argb(0,0,0,0));
		mAMap.setMyLocationStyle(my);
	}
	...
	public void onLocationChanged(AMapLocation amapLocation) {
		if (mListener != null && amapLocation != null) {
			if (amapLocation != null && amapLocation.getErrorCode() == 0) {
				...
				mAMap.moveCamera(CameraUpdateFactory.zoomTo(18));//按照传入的CameraUpdate参数改变地图状态。
				if (btn.isChecked()) {
	...
	public void record(View view) {
		Intent intent = new Intent(RecordPathActivity.this, RecordActivity.class);
		...
		if (lineID == 1) {
			if (linepoints != null && linepoints.size()>0) {
				...
				if (mlocMarker == null) {
					...
					Toast.makeText(RecordPathActivity.this, "距离"+mDistance, Toast.LENGTH_SHORT).show();
